...
const colorSelector = async () => {
  const color = await eyeDropper
    ...
    .then(async (colorValue) => {
      ...
      // Make a POST request to store color data in MongoDB
      await fetch('http://localhost:3000/storeColor', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ hexValue, rgbValue }),
      });
